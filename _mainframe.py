...
class Frame1(wx.Frame):
    ...
    def ObjectsBrowserSelChanged(self, event):
        ...
        obj = self.treeCtrl_ObjectsBrowser.GetItemData(tree_item)
        print("this is changed",tree_item,obj)
    ...
    def ObjectsBrowserRightClick(self, event):
        ...
        print("this is rightclick",tree_item,obj)
        if obj._check_existence():
    ...
    def _init_windows_tree(self):
        ...
        #item_data = wx.TreeItemData(proxy.PC_system(None))
        #print(item_data)

        root_obj = proxy.PC_system(None)
        item_data = wx.TreeItemData(root_obj)


        #item_data.SetData(root_obj)
        #print(root_obj.GetProperties())

        self.treeCtrl_ObjectsBrowser.AddRoot(root_obj.GetProperties()['PC name'], data =item_data)
        #self.treeCtrl_ObjectsBrowser.AddRoot(root_obj.GetProperties()['PC name'])
...
class tree_updater(object):
    ...
    def _update(self):
        ...
        print("this is tree_item",tree_item)
        print("this is obj",obj)

        subitems = obj.Get_subitems()
        print("this is subitems",subitems)
        for i_name, i_obj in subitems:
          item_data = wx.TreeItemData(i_obj)
